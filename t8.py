...
forecast_days = st.sidebar.slider("Days to Forecast", 1, 5, 3)
    ...
    if not df.empty:
        ...
        if len(df) < 30:
            ...
            try:
                ...
                lookback = 5  # Use 5 days of historical data
                ...
                for _ in range(forecast_days):
                    ...
                    current_features[1] = merged_data['Sentiment'].iloc[-1]  # Keep latest sentiment
                ...
                st.dataframe(
                    forecast_df.style.format({'Predicted Return %': '{:.2f}%'}),
                    use_container_width=True
                )
...
- **Machine Learning Forecasting:**
  - XGBoost regression model with multiple input factors:
    - Endogenous: Historical price/return patterns (5-day lagged returns)
    - Exogenous: News sentiment trends (5-day lagged sentiment scores)
    - Technical indicators: Moving average convergence/divergence
  - Recursive multi-step forecasting for 1-5 day predictions
